...
theorem commute_toEnd_casimir {B : LinearMap.BilinForm R L} (hB : B.lieInvariant L)
    {b b' : ι → L}
...
`Module.End R M`. -/
theorem lie_toEnd_casimir {B : LinearMap.BilinForm R L} (hB : B.lieInvariant L)
    {b b' : ι → L}
    (hb : ∀ y : L, ∑ i, B y (b' i) • b i = y)
    (hb' : ∀ y : L, ∑ i, B (b i) y • b' i = y)
    (x : L) :
...
theorem IsCasimirScalar.commute_toEnd {b b' : ι → L} {c : R}
...
theorem commute_toEnd_casimir_of_dual {B : LinearMap.BilinForm R L} (hB : B.lieInvariant L)
...
theorem commute_toEnd_casimirOfBasis {B : LinearMap.BilinForm R L} (hB : B.lieInvariant L)
