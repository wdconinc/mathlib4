...
@[simp]
theorem coord_lie_basis (i j k : ι) : b.coord k ⁅b i, b j⁆ = structureConstants b i j k := by
  simp [structureConstants]

/-- The structure constants vanish when their first two indices agree, since `⁅x, x⁆ = 0`. -/
@[simp]
theorem structureConstants_self (i k : ι) : structureConstants b i i k = 0 := by
  simp [structureConstants]

/-- The structure constants are antisymmetric in their first two indices. -/
theorem structureConstants_swap (i j k : ι) :
...
theorem lie_jacobi' (x y z : L) : ⁅⁅x, y⁆, z⁆ + ⁅⁅y, z⁆, x⁆ + ⁅⁅z, x⁆, y⁆ = 0 := by
...
theorem lie_basis_eq_sum (i j : ι) : ⁅b i, b j⁆ = ∑ k, structureConstants b i j k • b k :=
...
theorem coord_lie_eq_sum (x : L) (j k : ι) :
...
/-- The coordinates of `⁅x, b j⁆`, expressed via the coordinates of `x` and the structure
constants. -/
theorem repr_lie_eq_sum (x : L) (j k : ι) :
...
structure constants. -/
theorem coord_lie_lie_basis (i j k l : ι) :
...
identity. -/
theorem sum_structureConstants_cyclic_eq_zero (i j k l : ι) :
