...
* `Matrix.exp_conjTranspose` and `exp_mem_unitary_of_mem_skewAdjoint` together give the unitary half
  ...
  instance (`Mathlib/Topology/Instances/Matrix.lean`) is registered for the *Pi* topology; choosing a
  matrix norm in order to have `NormedSpace.exp` installs a different `TopologicalSpace` instance and
  the two do not connect by instance resolution. This is the norm-choice friction that
  `Mathlib/Analysis/Normed/Algebra/MatrixExponential.lean` documents in its own module docstring.
...
theorem mem_su_iff (A : Matrix n n ℂ) : A ∈ su n ↔ Aᴴ = -A ∧ Matrix.trace A = 0 := Iff.rfl

theorem conjTranspose_of_mem {A : Matrix n n ℂ} (h : A ∈ su n) : Aᴴ = -A := h.1

theorem trace_of_mem {A : Matrix n n ℂ} (h : A ∈ su n) : Matrix.trace A = 0 := h.2

theorem mem_skewAdjoint_of_mem_su {A : Matrix n n ℂ} (h : A ∈ su n) :
